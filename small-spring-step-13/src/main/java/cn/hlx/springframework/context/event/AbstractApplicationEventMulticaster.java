...
public abstract class AbstractApplicationEventMulticaster implements ApplicationEventMulticaster, BeanFactoryAware {
    ...
    protected boolean supportsEvent(ApplicationListener<ApplicationEvent> applicationListener, ApplicationEvent event) {
        ...
        // 通常Cglib代理生成的类, 是EnhancerByCGLIB类型, 而不是本身, 它的superClass才是本身
        // 按照 CglibSubclassingInstantiationStrategy-SimpleInstantiationStrategy 不同的实例化类型, 需要判断后获取目标 class
        Class<?> targetClass = ClassUtils.isCglibProxyClass(listenerClass) ? listenerClass.getSuperclass() : listenerClass;
        Type genericInterface = targetClass.getGenericInterfaces()[0]; // 获取实现的接口
        // 转为参数化类型<>泛型, 获取第一个泛型
        ...
        // isAssignableFrom是用来判断子类和父类的关系的, 或者接口的实现类和接口的关系的, 默认所有的类的终极父类都是Object, 如果是A.isAssignableFrom(B) 结果是true, 证明可以转换成为A, 也就是A可以由B转换而来
